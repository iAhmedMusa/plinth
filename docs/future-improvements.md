...
Production hardening additions that are not yet implemented. Each section
explains what it is, why it matters, how to build it, and what risk it
removes. Improvements that are already in place (Trivy image scanning,
secrets rotation via Secrets Manager, blue-green deployment support) are
not repeated here.
...
- **One-click rollback:** revert a Git commit and ArgoCD rolls back the
  cluster.
- **Audit trail:** every deployment is a Git commit with a作者, timestamp,
  and diff.
...
| # | Improvement | Risk removed | Effort |
|---|---|---|---|
| 1 | HPA autoscaling | Traffic spikes, cost waste | Low |
| 2 | GitOps (ArgoCD) | Drift, rollback errors | Medium |
| 3 | Canary deployments | Bad releases affecting all users | Medium |
| 4 | Centralized logging | Lost diagnostics, blind spots | Low |
| 5 | Image signing | Supply-chain attacks | Low |
| 6 | OPA Gatekeeper | Policy bypass via manual apply | Medium |
| 7 | Multi-region DR | Region-wide outage | High |
